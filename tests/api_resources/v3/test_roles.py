...
class TestRoles:
    ...
    def test_streaming_response_list(self, client: Qanapi) -> None:
        with client.v3.roles.with_streaming_response.list() as response:
...
class TestAsyncRoles:
    ...
    async def test_method_list(self, async_client: AsyncQanapi) -> None:
        role = await async_client.v3.roles.list()
        assert_matches_type(RoleListResponse, role, path=["response"])

    @pytest.mark.skip(reason="Mock server tests are disabled")
    @parametrize
    async def test_raw_response_list(self, async_client: AsyncQanapi) -> None:
        response = await async_client.v3.roles.with_raw_response.list()
    ...
    async def test_streaming_response_list(self, async_client: AsyncQanapi) -> None:
        async with async_client.v3.roles.with_streaming_response.list() as response:
            assert not response.is_closed
            assert response.http_request.headers.get("X-Stainless-Lang") == "python"
