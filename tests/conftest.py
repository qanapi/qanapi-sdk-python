...
from qanapi import Qanapi, AsyncQanapi, DefaultAioHttpClient
from qanapi._utils import is_dict
...
logging.getLogger("qanapi").setLevel(logging.DEBUG)
...
def client(request: FixtureRequest) -> Iterator[Qanapi]:
    strict = getattr(request, "param", True)
    if not isinstance(strict, bool):
        raise TypeError(f"Unexpected fixture parameter type {type(strict)}, expected {bool}")

    with Qanapi(base_url=base_url, api_key=api_key, subdomain=subdomain, _strict_response_validation=strict) as client:
        yield client


@pytest.fixture(scope="session")
async def async_client(request: FixtureRequest) -> AsyncIterator[AsyncQanapi]:
